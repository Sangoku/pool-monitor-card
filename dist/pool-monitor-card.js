...
`;var T=T||Object.getPrototypeOf(customElements.get("ha-panel-lovelace"));var C=T.prototype.html;class cardContent{static generateTitle(e){const i=e.title!==void 0?C` <h1 class="pool-monitor-title">${e.title}</h1> `:C``;return C`${i}`}static generateBody(e,i){if(!i)return C` <div class="warning-message">No sensor data available</div> `;return C`
      ...
      <div class="section" @click=${()=>cardContent._moreinfo(i.entity)}>
        <div class="pool-monitor-container-marker">
          ...
          <div
            class="triangle"
            style="border-top: 10px solid ${i.color} ;left: ${i.pct_marker}%;"
          ></div>
        </div>
        ${!i.hide_icon?C`
    ...
    `}static _moreinfo(e){const i=new Event("hass-more-info",{bubbles:true,composed:true});i.detail={entityId:e};const o=document.querySelector("home-assistant");if(o)o.dispatchEvent(i)}}var P=P||Object.getPrototypeOf(customElements.get("ha-panel-lovelace"));var M=P.prototype.html;console.info(`%c POOL-MONITORING-CARD %c ${f} `,b.title,b.version);class PoolMonitorCard extends P{static cardType=x.cardType;static cardName=x.cardName;static cardDescription=x.cardDescription;static get properties(){return{hass:{},config:{}}}static styles=k;constructor(){super()}render(){const e=this.getConfig();const i=this.processData();const o=e.display.compact?cardContent.generateCompactBody:cardContent.generateBody;if(!i||Object.keys(i).length===0)return M` <div id="pool-monitor-card">
      ...
      ${Object.values(i).map((i=>{if(i?.invalid)return M`
          ...
          `;return o(e,i)}))}
    </div>`}processData(){const e={};const i=this.getConfig();Object.entries(i.sensors).forEach((([i,o])=>{const r=Array.isArray(o)?o:[o];r.forEach(((o,r)=>{const a=`${i}_${r+1}`;e[a]=this.calculateData(i,o.title||this.getTranslatedText("sensor."+i),o.entity,o.min,o.max,o.setpoint,o.step,o.unit,o.icon,o.image_url,o.mode,o.min_limit,o.override_value,o.override,o.invalid)}))}));return e}getTranslatedText(e,i){const o=this.config?.display.language||"en";const r=getTranslation(o,e);return formatTranslation(r,i)}calculateData(e,i,o,r,a,s,n,l,c,d,p,m,u,g,h){const v={};const y=this.getConfig();const _=getSensorConfig(e)||{};v.name=e;v.invalid=h;v.mode=p;v.title=y.display.show_names?i:M`&nbsp;`;v.hide_icon=false;v.is_mdi=false;if(!y.display.show_icons)v.hide_icon=true;else{const i=c||"";const o=d||"";if(i==="hide")v.hide_icon=true;else if(o)v.img_src=o;else if(i&&typeof i==="string"&&i.startsWith("mdi:")){v.is_mdi=true;v.mdi_icon=i}else v.img_src=`https://raw.githubusercontent.com/wilsto/pool-monitor-card/master/resources/${e}.png`}if(!this.hass||!this.hass.states||!this.hass.states[o]){console.warn(`Entity not found: ${o}`);v.value=null;v.entity=o;return v}const f=this.hass.states[o];const x=f.attributes?.display_precision??f.attributes?.precision??this.countDecimals(parseFloat(f.state));const b=parseFloat(f.state);v.value=isNaN(b)?null:Number(b.toFixed(x));v.entity=o;if(y.display.show_last_updated)v.last_updated=this.timeFromNow(f.last_updated);v.unit=y.display.show_units?l||_.unit||"":"";if(g)v.value=u||_.override;v.min_value=r!==void 0&&this.hass.states[r]&&!isNaN(parseFloat(this.hass.states[r].state))?parseFloat(this.hass.states[r].state):v.value;v.max_value=a!==void 0&&this.hass.states[a]&&!isNaN(parseFloat(this.hass.states[a].state))?parseFloat(this.hass.states[a].state):v.value;s=s!==void 0?parseFloat(s):_.setpoint!==void 0?parseFloat(_.setpoint):v.value;n=n!==void 0?parseFloat(n):_.step!==void 0?parseFloat(_.step):.1;const w=Math.max(this.countDecimals(s),this.countDecimals(n));v.setpoint=s;const $=m!==void 0?Number(m):-1/0;const k=Math.max($,s-2*n);const T=Math.max($,s-n);const C=Math.max($,s);const P=Math.max($,s+n);const N=Math.max($,s+2*n);v.setpoint_class=[k.toFixed(w),T.toFixed(w),C.toFixed(w),P.toFixed(w),N.toFixed(w)];v.separator=y.display.show_labels?"-":"";v.color="transparent";if(v.value!==null)v.value=Math.max($,v.value);if(p==="heatflow")if(Number(v.value)<Number(v.setpoint_class[1])){v.state=y.display.show_labels?this.getTranslatedText("state.1"):"";v.color=y.colors.cool}else if(Number(v.value)>=Number(v.setpoint_class[1])&&Number(v.value)<Number(v.setpoint_class[3])){v.state=y.display.show_labels?this.getTranslatedText("state.3"):"";v.color=y.colors.low}else{v.state=y.display.show_labels?this.getTranslatedText("state.5"):"";v.color=y.colors.warn}else if(Number(v.value)<Number(v.setpoint_class[0])){v.state=y.display.show_labels?this.getTranslatedText("state.1"):"";v.color=y.colors.warn}else if(Number(v.value)>=Number(v.setpoint_class[0])&&Number(v.value)<Number(v.setpoint_class[1])){v.state=y.display.show_labels?this.getTranslatedText("state.2"):"";v.color=y.colors.low}else if(Number(v.value)>=Number(v.setpoint_class[1])&&Number(v.value)<Number(v.setpoint_class[2])){v.state=y.display.show_labels?this.getTranslatedText("state.3"):"";v.color=y.colors.normal}else if(Number(v.value)>=Number(v.setpoint_class[2])&&Number(v.value)<Number(v.setpoint_class[3])){v.state=y.display.show_labels?this.getTranslatedText("state.4"):"";v.color=y.colors.normal}else if(Number(v.value)>=Number(v.setpoint_class[3])&&Number(v.value)<Number(v.setpoint_class[4])){v.state=y.display.show_labels?this.getTranslatedText("state.5"):"";v.color=y.colors.low}else if(Number(v.value)>=Number(v.setpoint_class[4])){v.state=y.display.show_labels?this.getTranslatedText("state.6"):"";v.color=y.colors.warn}v.progressClass=e==="temperature"?"progress-temp":"progress";v.pct=Math.max(0,Math.min(98.5,Math.max(0,v.value-(s-3*n))/(6*n)*.85*100+15)).toFixed(0);v.pct_min=Math.max(0,Math.min(98.5,Math.max(0,v.min_value-(s-3*n))/(6*n)*.85*100+15)).toFixed(0);v.pct_max=Math.max(0,Math.min(98.5,Math.max(0,v.max_value-(s-3*n))/(6*n)*.85*100+15)).toFixed(0);v.pct_marker=v.value>v.setpoint?v.pct-12:v.pct-5;v.side_align=v.value>s?"right":"left";v.pct_cursor=v.value>s?100-parseFloat(v.pct):parseFloat(v.pct)-2;v.pct_state_step=v.value>s?105-parseFloat(v.pct):parseFloat(v.pct)+5;v.pct_min=v.value>s?100-parseFloat(v.pct_min):parseFloat(v.pct_min)-2;v.pct_max=v.value>s?100-parseFloat(v.pct_max):parseFloat(v.pct_max)-2;return v}countDecimals(e){if(e===void 0||e===null)return 0;if(Math.floor(e)===e)return 0;const i=e.toString();if(i.includes("."))return i.split(".")[1].length||0;return 0}timeFromNow(e){const i=new Date(e);const o=Date.now()-i.getTime();const t=(e,i)=>{const o=i===1?"time":"time_plural";const r={[e]:i};return this.getTranslatedText(`${o}.${e}`,r)};const r=Math.floor(o/6e4);const a=Math.floor(r/60);const s=Math.floor(a/24);if(r<1)return t("seconds",0);if(r<60)return t("minutes",r);if(a<24)return t("hours",a);return t("days",s)}getConfig(){return this.config}setConfig(e){const i={display:getDisplayConfig(),colors:getColorConfig()};const o={...e,display:{...i.display,...e.display||{}},colors:{...i.colors,...e.colors||{}},sensors:{}};if(!e.sensors){console.warn(`Legacy configuration detected, please move sensors under "sensors" key`,{config:e});throw new Error('Legacy configuration detected. Please update your setup to define all sensors under the "sensors" key as required by the version 2.0 of the card.')}Object.entries(e.sensors).forEach((([i,r])=>{const a=getSensorConfig(i);const s=Array.isArray(r)?[...r]:[{...r}];if(s.length===0)throw new Error(`Empty sensor array for ${i}`);const n=s.map((e=>({...a,...e,nameDefinedByUser:!!e.name})));n.forEach(((o,r)=>{if(!o.entity)throw new Error(`Missing entity for ${i}[${r}]`);if(o.nameDefinedByUser)o.title=o.name;if(!v.includes(i)){console.warn(`Unsupported sensor type: ${i}`,{sensorType:i,supportedSensors:v,config:e,sensorConfig:o});o.invalid=true}else o.invalid=false}));o.sensors[i]=n}));this.config=o}}customElements.define("pool-monitor-card",PoolMonitorCard);e.PoolMonitorCard=PoolMonitorCard;Object.defineProperty(e,Symbol.toStringTag,{value:"Module"});return e}({});
